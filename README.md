...
`agent-skills.toml` declares the personal Claude/Codex agent skills that belong in `~/.agents/skills/` - each `[name]` section names a GitHub `source` repo and an optional `path` within it (defaults to `.`). Installed skill folders and generated state under `~/.agents/`, plus plugin caches, auth, and sessions, are never tracked by this repo; only the manifest is.
