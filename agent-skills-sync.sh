...
#
# --dry-run prints the planned clone/copy per skill without touching the
# network or the filesystem. --dest lets tests point at a scratch directory
...
for name in "${AGENT_SKILL_NAMES[@]}"; do
  ...
  [ -d "$src_path" ] || { echo "agent-skills: $name: path '$path' not found in $repo" >&2; rm -rf "$tmp"; exit 1; }
  [ -f "$src_path/SKILL.md" ] || { echo "agent-skills: $name: no SKILL.md under '$path' in $repo" >&2; rm -rf "$tmp"; exit 1; }
