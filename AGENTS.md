...
- In `home.nix`, the `gitverify` alias (`config.home.homeDirectory` instead of a hardcoded `/Users/` path) and the `ai-fill-buffer` system prompt (no longer says "for macOS") are deliberately left platform-unconditional, not gated behind `pkgs.stdenv.isDarwin`/`isLinux`. Both resolve to identical runtime behavior on macOS, so this trades a byte-identical `darwinConfigurations.mac` derivation for a simpler shared expression. `tests/flake-linux.test.sh` asserts the mac output still evaluates, not that it's byte-identical to the pre-Linux baseline - do not "fix" that test to gate these lines.
