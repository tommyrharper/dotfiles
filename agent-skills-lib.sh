...
agent_skills_parse_manifest() {
	...
	[ -f "$file" ] || {
		echo "agent-skills: manifest not found: $file" >&2
		return 1
	}
	...
	while IFS= read -r line; do
		...
		if [[ "$line" =~ ^([A-Za-z0-9_]+)[[:space:]]*=[[:space:]]*\"(.*)\"$ ]]; then
			...
			val="${BASH_REMATCH[2]}"
			[ -z "$section" ] && {
				echo "agent-skills: $file: '$key' outside any [section]" >&2
				return 1
			}
			case "$key" in
			...
			path)
				# shellcheck disable=SC2034
				AGENT_SKILL_PATH["$section"]="$val"
				;;
			*)
				echo "agent-skills: $file: unknown key '$key' in [$section]" >&2
				return 1
				;;
			esac
			continue
	...
	for section in "${AGENT_SKILL_NAMES[@]}"; do
		[ -n "${AGENT_SKILL_SOURCE[$section]:-}" ] || {
			echo "agent-skills: $file: [$section] missing 'source'" >&2
			return 1
		}
	done
}
